...
//      ref.current.click();
  ...
  return (
        ...
        <Container className="relative w-full rounded-md bg-muted/10 p-2 text-left shadow-none hover:cursor-pointer hover:bg-muted/20" onClick={()=> handleClick(id)}>
          <Heading>{title}</Heading>
          <Text className="text-muted-foreground">{description}</Text>
          {Boolean(labels.length) && (
            <div className="m-1 gap-1">
              {labels.map((label: Label) => (
                <Badge key={label.label} color={'grey'}>{label.label.toUpperCase()}</Badge>
              ))}
            </div>
          )}
          <Button
