...
function IdeaCard(props: IdeaCardProps) {
  ...
  const truncatedTitle = useMemo(() => title.substring(0, MAX_TITLE_LENGTH) + '...', [title]);
  const truncatedDescription = useMemo(() => description.substring(0, MAX_DESCRIPTION_LENGTH) + '...', [description]);
  ...
  return (
    <AlertDialog>
      <AlertDialogTrigger asChild>
        <Card className="w-full hover:bg-secondary/40 hover:cursor-pointer pt-2">
